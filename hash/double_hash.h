...
class double_hash{
    public:
        ...
        friend ostream& operator <<(ostream& outs, const double_hash<T>& print_me){
            bool flag;
    ...
    private:
        ...
        size_t hash_again(int key) const;
...
template<class T>
int double_hash<T>::check_col(int index, int key, bool& check) const{
...
template <class T>
inline size_t double_hash<T>::hash(int key) const{
    return (key % CAPACITY);
}

template<class T>
inline size_t double_hash<T>::hash_again(int key)const{
...
template <class T>
void double_hash<T>::find_index
(int key, bool& found, size_t& i) const{
...
template <class T>
inline bool double_hash<T>::never_used(size_t index) const{
    return (data[index]._key == NEVER_USED);
}

template <class T>
inline bool double_hash<T>::is_vacant(size_t index) const{
